...
Singleton {
    ...
    // Conteo para el badge de la barra, independiente de 'entries' para que en
    // reposo baste un recuento barato sin retener el historial completo.
    ...
    // Arranca el vigilante una sola vez, cuando Deps termina la detección. Cubre
    // los dos órdenes posibles: Deps ya listo al crearse este singleton, o su
    // señal llegando después.
    ...
    function remove(entry) {
        ...
        // Baja local optimista: el ScriptModel del panel ve un único borrado y
        // las filas de debajo suben animadas, en vez de reconstruirse de golpe
        // perdiendo la posición de scroll.
        //
        // Se compara por VALOR y nunca por identidad: el modelData del delegate
        // llega como copia tras pasar por ScriptModel, así que comparar
        // referencias no encontraría nada y el borrado no llegaría al modelo.
    ...
    // Vigila el portapapeles: en cada copia real almacena con cliphist y emite
    // el recuento por stdout, en un solo proceso. También emite un recuento
    // inicial para el badge, y con el panel abierto recarga la lista completa.
    Process {
        id: watcher
        running: false
        command: ["sh", "-c",
        ...
        // Si el vigilante muere se relanza tras una pausa.
    ...
    // para que la animación de cierre no se vea vaciarse. Al reabrir, el panel
    // llama a refresh().
    Timer {
        id: purgeTimer
        interval: 600

...
// Tarjeta de notificación, usada tanto en los popups como en el centro.
//
// Mapeo de roles Material a Theme: Surface→bg, SurfaceVariant→surface,
// OnSurface→fg, OnSurfaceVariant→fgDim, Outline→overlay, Primary→accent. Las
// medidas van en píxeles lógicos pasadas por Theme.dp() y sp(), para respetar la
// escala de densidad y de fuente del shell.
Rectangle {
    id: item
    property var notif
    ...
    // Modo compacto: solo título, para que una tanda de avisos ocupe una franja
    // fina. No afecta al centro de notificaciones, donde sí se quiere leer entero.
    property bool compact: false
    // Modo denso: la misma tarjeta, apretada. Esta está medida para el popup, que
    // es ancho, y en la hoja de la isla un título grande junto a un icono grande
    // se comen la fila entera.
    //
    // Se hace con una propiedad y no con un archivo aparte a propósito: el aviso
    // tiene que verse igual en los dos sitios —misma anatomía, mismo orden,
    // mismos gestos— solo que más apretado. Dos archivos serían dos diseños que
    // se separan al primer cambio.
    property bool dense: false

    // Primer plano, todo menos el fondo de la tarjeta. El barrido de entrada
    // descubre el fondo ya opaco y solo el contenido funde con retardo y se
    // contra-desplaza, por eso opacidad y desplazamiento van aquí.
    ...
    // Urgencia: tiñe la barra de progreso.
    ...
    // Acción "default": la tarjeta entera es su área de clic.
    ...
    // Anchos de texto calculados a partir del ancho de la tarjeta en vez de
    // dejarlos en manos de Layout.fillWidth. No es cosmética: con fillWidth, en
    // la primera pasada de layout los anchos valen 0, el título envuelve a dos
    // líneas y la tarjeta mide de más. El ListView de los popups mide justo en
    // ese instante y deja congelada la posición de las de abajo.
    ...
    // La tarjeta se pinta con el tono del fondo del shell, apenas translúcido:
    // flotando sobre el escritorio eso basta para que se lea como una tarjeta.
    //
    // Apretada no, por la razón inversa: ahí va dentro de la hoja de la isla, que
    // ya es una superficie oscura, y una tarjeta del tono del fondo sobre un
    // fondo oscuro se ve como un recuadro dibujado a lápiz. Sube un peldaño de la
    // escalera de contenedores, que es para lo que existe.
    ...
    // Clic izquierdo invoca la acción "default" y cierra; clic derecho descarta.
    // Va detrás del contenido, así que los botones de acción y el de cerrar se lo
    // comen primero.
    MouseArea {
        anchors.fill: parent
        acceptedButtons: Qt.LeftButton | Qt.RightButton
        cursorShape: item.hasDefaultAction ? Qt.PointingHandCursor : Qt.ArrowCursor
        // invoke() cierra el aviso, y la tarjeta de la isla cuelga de la lista
        // del servicio: se destruye dentro de esta misma llamada, así que nada
        // que toque 'item' puede ir detrás. El popup vive en su propio modelo y
        // sobrevive, pero hay que retirarlo a mano; se pide antes de invocar,
        // porque dismiss() ahí solo lanza la animación de salida y la fila
        // sigue en el modelo hasta que termina.
        onClicked: (mouse) => {
    ...
    ColumnLayout {
        ...
        // Barra de timeout, pegada al borde superior. El relleno es una pastilla
        // centrada que se encoge hacia el medio desde ambos extremos.
        Item {
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                // Antetítulo, solo en modo denso: quién avisa y cuándo, en
                // pequeño y arriba. En la tarjeta ancha esto va en el pie, pero
                // apretada ese pie queda como una palabra suelta colgando. Delante
                // del título hace de encabezado y la fila se lee de un vistazo.
                ThemedText {
                ...
                // Título, reservando a su derecha el hueco del botón de cerrar.
                ThemedText {
                    Layout.preferredWidth: item.titleWidth
                    text: item.summary
                    color: Theme.fg
                    font.pixelSize: Theme.sp(item.dense ? 13 : 16)   // fontSizeTitle
                    // Apretada baja de peso: en negrita y sobre un antetítulo
                    // gris, el título gritaría dos veces.
                ...
                // Acciones: se reparten en varias filas si no caben.
                Flow {
                    Layout.preferredWidth: item.textColWidth
                    ...
                    Repeater {
                        ...
                        delegate: Rectangle {
                            ...
                            // La acción "default" es la tarjeta entera, no un
                            // botón.
                ...
                // Pie con el nombre de la app. El popup no lleva marca de
                // tiempo; solo la lista del historial.
                ThemedText {
                    Layout.preferredWidth: item.textColWidth
    ...
    // Botón de cerrar: fantasma, en la esquina superior derecha y con posición
    // absoluta.
    Rectangle {
        id: closeButton

...
En `show interfaces Gi1/0/12` importan los contadores: `CRC` y `input
errors` subiendo son cable o dúplex, `late collisions` es casi seguro un
dúplex mal negociado (un lado fijo y otro auto).
...
configurado: se investiga la causa (¿qué MAC lo disparó? `show
port-security interface …`), se arregla, y se rearma con `shutdown` +
`no shutdown` en la interfaz. Rearmar sin arreglar la causa es un bucle.
...
- Los equipos viejos solo hablan SSH con algoritmos antiguos: si el ssh
  modero se niega, se añaden `-oKexAlgorithms=+diffie-hellman-group14-sha1

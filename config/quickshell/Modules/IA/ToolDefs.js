...
function core() {
    return [
        ...
        { type: "function", "function": {
            name: "subagent",
            description: "Delega una tarea en un subagente autónomo de solo lectura (busca, lee archivos y páginas, consulta el sistema y servidores, y devuelve un informe). Úsalo para exploraciones largas que ensuciarían esta conversación; lo que haya que escribir o ejecutar te toca a ti después. Puedes lanzar VARIOS a la vez en el mismo turno (hasta 4) y corren en paralelo: reparte una búsqueda amplia en trozos.",
            parameters: { type: "object",
                properties: { task: { type: "string", description: "El encargo, con todo el contexto necesario" },
                              ...
                              role: { type: "string", "enum": ["research", "review", "debug"],
                                      description: "research (rastrear, por defecto), review (revisar código), debug (diagnosticar una avería)" },
                              output: { type: "string", description: "Forma que debe tener el informe, p. ej. 'lista de hallazgos con archivo:línea y arreglo'" },
                              max_rounds: { type: "integer", description: "Tope de rondas de herramientas (1-12, por defecto 8)" } },
                required: ["task"] } } },
        { type: "function", "function": {
...
function dev() {
    return [
        ...
        { type: "function", "function": {
            name: "debug_start",
            description: "Arranca el programa del usuario bajo un DEPURADOR de verdad (debugpy para Python, lldb-dap para binarios nativos, delve para Go) con puntos de ruptura. Contesta cuando el programa pare en una ruptura (o termine) con la pila y la salida. Después: debug_view para mirar, debug_ctl para avanzar, debug_eval para evaluar expresiones en el marco parado.",
            parameters: { type: "object",
                properties: { program: { type: "string", description: "Script .py, .go o binario, dentro de la carpeta personal" },
                              ...
                              lang: { type: "string", "enum": ["python", "go", "native"],
                                      description: "Solo si la extensión no lo dice" } },
                required: ["program"] } } },
        { type: "function", "function": {
            name: "debug_ctl",

...
for (const f of ["core/ChatClient.qml", "core/Compactor.qml",
    ...
    comprueba(f.split("/").pop() + ": llama al transporte", llamadas > 0, String(llamadas));
    // Nunca directo a .command: eso es lo que rompió el supervisor.
    comprueba(f.split("/").pop() + ": no asigna el objeto a command",
        !/\.command\s*=\s*(?:svc|AiService|ai)\.(?:chatCommand|probeCommand)/.test(src));
    comprueba(f.split("/").pop() + ": reparte cmd y env", /\.command\s*=\s*\w+\.cmd/.test(src)
    ...
    comprueba(f.split("/").pop() + ": guarda el cuerpo", conCuerpo || f.indexOf("ConnectionProbe") !== -1);
    if (conCuerpo) {
        comprueba(f.split("/").pop() + ": lo escribe en onStarted", /onStarted[\s\S]{0,200}\.write\(/.test(src));
        comprueba(f.split("/").pop() + ": y cierra la entrada", /stdinEnabled\s*=\s*false/.test(src));
    }
}

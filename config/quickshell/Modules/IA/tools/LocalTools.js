...
function sysQuery(tool, args, ctx) {
    ...
    case "network_query": {
        ...
        // Quién hay en la red de casa, SIN barrerla. Sale de la tabla de
        // vecinos que el núcleo ya tiene de haber hablado con ellos: es
        // instantáneo, no manda un solo paquete y no se parece a un escaneo.
        // Existe porque el modelo, al no tenerlo, se inventó un bucle de 254
        // pings en paralelo — cuando falta la herramienta honesta, improvisa
        // una peor.
        case "neighbors":  return { cmd: ["sh", "-c",
            'ip neigh show | grep -v FAILED | awk \'{printf "%-16s %-20s %s\\n", $1, $5, $6}\' | sort -V | head -n 40; '
            + 'echo; echo "(de la tabla de vecinos del núcleo: solo aparece quien ya ha hablado con esta máquina)"'], env: {} }
        case "ping": {
        ...
        return { error: "kind debe ser interfaces, routes, ports, neighbors o ping." }
    }
    case "disk_query": {
...
// Los enlaces de una página, resueltos a absolutos y sin los menús. Lo que
// permite NAVEGAR un sitio en vez de quedarse en la primera página: seguir a
// una sección, mirar el sitemap, ir al siguiente resultado. Sin esto, el
// modelo lo hacía con curl y grep — está en el registro de auditoría.
const PY_ENLACES = [
    'import html as _h',
    'import os',
    'import re',
    ...
    '',
    'BASE = os.environ.get("QS_BASE", "")',
    'crudo = sys.stdin.read()',
    '# Fuera lo que no es contenido: los menús viven casi siempre ahí dentro.',
    'cuerpo = re.sub(r"(?is)<(script|style|nav|header|footer|svg)\\b.*?</\\1>", " ", crudo)',
    '',
    'vistos, filas = set(), []',
    'for m in re.finditer(r\'(?is)<a\\b[^>]*?href\\s*=\\s*["\\\']([^"\\\']+)["\\\'][^>]*>(.*?)</a>\', cuerpo):',
    '    href = _h.unescape(m.group(1).strip())',
    '    if not href or href[0] == "#":',
    '        continue',
    '    if re.match(r"(?i)^(javascript|mailto|tel|data):", href):',
    '        continue',
    '    url = urljoin(BASE, href) if BASE else href',
    '    if urlsplit(url).scheme not in ("http", "https"):',
    '        continue',
    '    url = url.split("#")[0]',
    '    if url in vistos or url == BASE:',
    '        continue',
    '    vistos.add(url)',
    '    texto = " ".join(_h.unescape(re.sub(r"(?s)<[^>]+>", " ", m.group(2))).split())',
    '    if len(texto) > 70:',
    '        texto = texto[:70].rstrip() + "…"',
    '    filas.append((texto or "(sin texto)", url))',
    '',
    'if filas:',
    '    # Los del mismo sitio primero: navegar es casi siempre quedarse dentro.',
    '    casa = urlsplit(BASE).netloc if BASE else ""',
    '    filas.sort(key=lambda f: (urlsplit(f[1]).netloc != casa,))',
    '    print("\\n\\n── Enlaces de esta página (%d) ──" % len(filas))',
    '    for texto, url in filas[:80]:',
    '        print("· %s → %s" % (texto, url))',
    '    if len(filas) > 80:',
    '        print("… y %d más." % (len(filas) - 80))'
].join("\n")
...
const FETCH_SH = [
    ...
    'esac',
    // ── SOLO LA CABECERA ────────────────────────────────────────────────────
    // "¿Existe esta URL? ¿A dónde lleva? ¿es una página o un PDF?" era, tal
    // cual, `curl -s -o /dev/null -w "%{http_code}"` escrito a mano en el shell
    // por el modelo. Todo eso ya lo tenemos del bucle de saltos: es contarlo.
    //
    // Se pide igualmente con GET y no con HEAD a propósito: hay bastantes
    // servidores que contestan 405 a un HEAD y dirían "no existe" de algo que
    // sí existe. El cuerpo ya viene acotado a 2 MB y aquí se tira sin leerlo.
    'if [ "$QS_HEAD" = 1 ]; then',
    '  printf "%s\\n" "HTTP $code"',
    '  [ -n "$ct" ] && printf "Tipo: %s\\n" "$ct"',
    '  printf "Tamaño: %s bytes\\n" "$(wc -c < "$f" 2>/dev/null || echo 0)"',
    '  [ "$u" != "$QS_U" ] && printf "Acabó en: %s\\n" "$u"',
    '  [ "$salto" -gt 0 ] && printf "Redirecciones: %s\\n" "$salto"',
    '  exit 0',
    'fi',
    ...
    '[ -n "$aviso" ] && printf "\\n\\n%s\\n" "$aviso"',
    // ── LOS ENLACES ─────────────────────────────────────────────────────────
    // El extractor los tira a propósito: treinta enlaces de barra lateral son
    // ruido. Pero tirarlos SIEMPRE dejaba al modelo sin poder NAVEGAR un sitio,
    // y en el registro se ve lo que hacía entonces: `curl -o /tmp/p.html` y
    // luego `grep -oiE 'href="[^"]*"'`. O sea, reimplementar esto a mano, con
    // la página cruda entrando en su contexto de paso.
    //
    // Bajo petición (links=true), y después del texto: quien solo quiere leer
    // no paga por ellos.
    '[ "$QS_LINKS" = 1 ] && QS_BASE="$u" python3 -c "$QS_ENL" < "$f"',
    'exit 0'
].join("\n")
    ...
    case "fetch_url": {
        ...
        return { cmd: ["sh", "-c", FETCH_SH],
                 env: { QS_U: url, QS_U0: url, QS_LAN: "", QS_RES: PY_RESOLVE,
                        ...
                        QS_HEAD: args.head === true ? "1" : "",
                        QS_LINKS: args.links === true ? "1" : "",
                        QS_ENL: args.links === true ? PY_ENLACES : "",
...
// EL COMANDO LIBRE, con las tres cosas que le faltaban.
//
//   · La carpeta de trabajo, si la piden (ya comprobada por quien llama).
//   · El comando entra por el ENTORNO, no por el argv: en el argv lo leía
//     cualquier proceso de la máquina mientras corría, y un comando puede
//     llevar dentro una contraseña que el usuario acaba de dictar.
//   · Y se le quitan del entorno las variables que huelen a credencial. Hoy no
//     hay ninguna en el entorno de Quickshell —comprobado, veinte variables y
//     ninguna sospechosa—, pero eso es suerte y no diseño: basta arrancar el
//     shell desde una terminal con un `export ANTHROPIC_API_KEY=…` para que un
//     `env` del modelo se la lleve. SSH_AUTH_SOCK se salva a mano porque casa
//     con el patrón y no es un secreto, es la ruta de un zócalo — quitarla
//     rompería cualquier `git push`.
const SH_MANDATO = [
    '[ -n "$QS_CWD" ] && cd -- "$QS_CWD"',
    'for v in $(env | sed -n \'s/^\\([A-Za-z_][A-Za-z0-9_]*\\)=.*/\\1/p\' '
        + '| grep -Ei "(KEY|TOKEN|SECRET|PASS|CRED|AUTH)" '
        + '| grep -Ev "^(SSH_AUTH_SOCK|GPG_AGENT_INFO|SSH_AGENT_PID)$"); do',
    '  unset "$v"',
    'done',
    // Y el propio comando deja de estar en el entorno antes de correr: si lleva
    // dentro una contraseña, no tiene por qué heredarla cada proceso hijo y
    // quedar a la vista en su /proc/<pid>/environ. Como variable del shell, sin
    // exportar, la ve el intérprete y nadie más.
    'orden=$QS_CMD; unset QS_CMD',
    'sh -c "$orden"'
].join("\n")
